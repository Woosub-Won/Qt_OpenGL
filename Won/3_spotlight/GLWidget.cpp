...
void GLWidget::initializeGL()
{
    ...
    // 조명 설정
    lightPosition = QVector4D(0.0f, 10.0f, 0.0f, 1.0f);
    lightIntensity = QVector3D(1.0f, 1.0f, 1.0f); 
    lightDirection = QVector3D(0.0f, 0.0f, -1.0f);
    lightExponent = 2.0f;                                   // 감쇠 정도 감소
    cutoff = 15.0f;                                        // 더 좁은 각도

    // 재질 설정
    materialAmbient = QVector3D(0.56f, 0.32f, 0.16f);   // 구리 색상
...
void GLWidget::paintGL()
{
    ...
    // 공통 사용 uniform
    shaderProgram.setUniformValue("Spot.position", lightPosition);
    shaderProgram.setUniformValue("Spot.intensity", lightIntensity);
    shaderProgram.setUniformValue("Spot.direction", lightDirection);
    shaderProgram.setUniformValue("Spot.exponent", lightExponent);
    shaderProgram.setUniformValue("Spot.cutoff", cutoff);
    ...
    shaderProgram.setUniformValue("Shininess", materialShininess);
