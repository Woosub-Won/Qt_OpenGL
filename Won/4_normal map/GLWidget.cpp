...
GLWidget::GLWidget(QWidget* parent, Qt::WindowFlags f) :
    ...
    tangentBuffer(QOpenGLBuffer::VertexBuffer),
...
void GLWidget::initializeGL()
{
    ...
    // 재질 설정
    materialAmbient = QVector3D(0.2f, 0.2f, 0.2f);   // 구리 색상
    materialDiffuse = QVector3D(0.8f, 0.8f, 0.8f);
    ...
    texture1 = new QOpenGLTexture(QImage("bricks_color.png").mirrored());
    texture1->setMinificationFilter(QOpenGLTexture::LinearMipMapLinear);
    texture1->setMagnificationFilter(QOpenGLTexture::Linear);
    texture1->setWrapMode(QOpenGLTexture::Repeat);

    texture2 = new QOpenGLTexture(QImage("bricks_normal.png").mirrored());
    ...
    shaderProgram.setAttributeBuffer(vertexTex, GL_FLOAT, 0, 2, 0);

    tangentBuffer.create();
    tangentBuffer.bind();
    tangentBuffer.allocate(tangents.data(), tangents.size() * sizeof(float));
    GLuint vertexTangent = shaderProgram.attributeLocation("VertexTangent");
    shaderProgram.enableAttributeArray(vertexTangent);
    shaderProgram.setAttributeBuffer(vertexTangent, GL_FLOAT, 0, 4, 0);  // vec4로 설정
...
void GLWidget::readObjFile(const char* fileName)
{
    ...
    for (int i = 0; i < idx_v.size(); i++)
    {
        ...
        texCoord.push_back(tmp_t[idx_t[i] * 2 + 0]);
        texCoord.push_back(tmp_t[idx_t[i] * 2 + 1]);
    ...
    // 탄젠트 계산
    tangents.resize(vertices.size() * 4 / 3);  // vec4를 저장하므로 4/3배

    // 각 삼각형마다 탄젠트 계산
    for (size_t i = 0; i < indices.size(); i += 3) {
        // 삼각형의 세 정점
        QVector3D pos1(vertices[indices[i] * 3], vertices[indices[i] * 3 + 1], vertices[indices[i] * 3 + 2]);
        QVector3D pos2(vertices[indices[i + 1] * 3], vertices[indices[i + 1] * 3 + 1], vertices[indices[i + 1] * 3 + 2]);
        QVector3D pos3(vertices[indices[i + 2] * 3], vertices[indices[i + 2] * 3 + 1], vertices[indices[i + 2] * 3 + 2]);

        // 텍스처 좌표
        QVector2D uv1(texCoord[indices[i] * 2], texCoord[indices[i] * 2 + 1]);
        QVector2D uv2(texCoord[indices[i + 1] * 2], texCoord[indices[i + 1] * 2 + 1]);
        QVector2D uv3(texCoord[indices[i + 2] * 2], texCoord[indices[i + 2] * 2 + 1]);
        ...
        QVector3D tangent;
        tangent.setX(f* (deltaUV1.x()* edge2.x() - deltaUV2.x() * edge1.x()));
        tangent.setY(f* (deltaUV1.x()* edge2.y() - deltaUV2.x() * edge1.y()));
        tangent.setZ(f* (deltaUV1.x()* edge2.z() - deltaUV2.x() * edge1.z()));
        tangent.normalize();
