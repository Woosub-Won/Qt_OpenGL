#include "ch2_2_implementingdiffuse.h"
...
void ch2_2_ImplementingDiffuse::initializeGL()
{
    ...
    const GLchar* vertShaderCode = loadShaderAsString("C:/Users/user/Desktop/QtStudy/QtOpenGL_Jongmin/Shaders/vertShader2_2.vert");
    glShaderSource(vertShader, 1, &vertShaderCode, nullptr);
    glCompileShader(vertShader);

    const GLchar* fragShaderCode = loadShaderAsString("C:/Users/user/Desktop/QtStudy/QtOpenGL_Jongmin/Shaders/fragShader2_2.frag");
    ...
    QVector3D kd(1.0f, 0.5f, 0.2f);                 // 디퓨즈 반사 계수 (빨간색)
    QVector3D ld(1.0f, 1.0f, 1.0f);                 // 광원의 강도 (흰색)

    QMatrix4x4 modelViewMatrix;
    modelViewMatrix.translate(0.0f, 0.0f, -5.0f); // 카메라 위치 설정 (5번과 연관)
    ...
    glUniform4fv(glGetUniformLocation(programHandle, "LightPosition"), 1, &lightPosition[0]);
    glUniform3fv(glGetUniformLocation(programHandle, "Kd"), 1, &kd[0]);
    glUniform3fv(glGetUniformLocation(programHandle, "Ld"), 1, &ld[0]);
}

void ch2_2_ImplementingDiffuse::paintGL()
{
...
void ch2_2_ImplementingDiffuse::generateTorus(std::vector<float> &positions, std::vector<float> &normals, float outerRadius, float innerRadius, int numSides, int numRings)
{
    positions.clear();
    normals.clear();
...
const char *ch2_2_ImplementingDiffuse::loadShaderAsString(const QString &filePath)
{
    QFile file(filePath);
