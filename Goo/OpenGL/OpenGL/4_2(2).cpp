...
int main() {
	...
	Shader myShader = Shader("shader/Chapt4_2(2).vert", "shader/Chapt4_2(2).frag");
	myShader.use();
	...
	GLuint textureIDs[2];
	createTexture("texture/container2.png", textureIDs[0]);
	myShader.setInt("Tex1", 0);
	createTexture("texture/awesomeface.png", textureIDs[1]);
	...
	while (!glfwWindowShouldClose(window)) {
		...
		float time = glm::sin(currentFrame);
		if (time < 0) {
			time = -time;
		}
