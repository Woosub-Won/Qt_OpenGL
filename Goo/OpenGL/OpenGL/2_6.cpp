...
int main() {
	...
	Shader myShader = Shader("shader/Chapt2_6.vert", "shader/Chapt2_6.frag");
	myShader.use();
	...
	while (!glfwWindowShouldClose(window)) {
		...
		GLuint adsIndex = glGetSubroutineIndex(myShader.ID, GL_VERTEX_SHADER, "phongModel");

		GLuint diffuseIndex = glGetSubroutineIndex(myShader.ID, GL_VERTEX_SHADER, "diffuseOnly");

		// 서브루틴으로 셰이더의 실행을 동적으로 제어하는 함수
		// 유니폼 변수에 서브루틴 인덱스를 전달함으로써 동작함
		// 매개변수 
		// 1. 셰이더 타입
		// 2. 전달할 서브루틴 인덱스 개수
		// 3. 서브루틴 인덱스 배열
